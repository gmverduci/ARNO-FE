...
pazienteSelezionato$ = this.pazienteSelezionatoSub.asObservable();
  ...
  private handleError(error: HttpErrorResponse) {
    if (error.error instanceof ErrorEvent) {
      ...
      console.error(`Errore lato server: ${error.status}, ` + `messaggio: ${error.error.message}`);
    }
    return throwError('Qualcosa è andato storto. Riprova più tardi.');
  }

  private caricaPazienti(): void {
    this.http.get<Paziente[]>(this.apiURL)
      .pipe(
        catchError(this.handleError)
      )
      .subscribe(
        pazienti => this.pazientiSub.next(pazienti),
        error => console.error('Errore durante il recupero dei pazienti:', error)
      );
  }
  ...
  private updatePazientiSub() {
    this.getAllPazienti().subscribe(
      ...
      (error: any) => console.error('Errore durante l\'aggiornamento dei pazienti:', error)
    );
  }
  ...
  updatePaziente(id: number, pazienteDTO: ModificaPaziente): Observable<ModificaPaziente> {
    const url = `${this.apiURL}/${id}`;
    return this.http.put<ModificaPaziente>(url, pazienteDTO, { responseType: 'text' as 'json' })
      .pipe(
        catchError(this.handleError),
